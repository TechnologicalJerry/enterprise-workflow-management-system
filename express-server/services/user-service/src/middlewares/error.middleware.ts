...
export const errorMiddleware = (
  ...
  if (statusCode >= 500) logger.error('Server error', logData);
  else logger.warn('Client error', logData);
  ...
  if (!errorResponse.error.details) delete errorResponse.error.details;
  if (!errorResponse.error.stack) delete errorResponse.error.stack;
