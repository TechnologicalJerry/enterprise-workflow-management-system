...
export const errorMiddleware = (err: Error, req: Request, res: Response, _next: NextFunction) => {
  ...
  res.status(statusCode).json({ success: false, error: { code: errorCode, message: err.message, details: err instanceof HttpError ? err.details : undefined }, timestamp: new Date().toISOString(), correlationId: req.correlationId });
};
